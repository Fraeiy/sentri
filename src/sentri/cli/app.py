...
from sentri.cli import groups, users
...
def version() -> None:
    ...
    console.print(f"sentri v{__version__}")


@app.command()
def web(
    host: str | None = typer.Option(None, help="Bind host (default: 127.0.0.1)"),
    port: int | None = typer.Option(None, help="Bind port (default: 8080)"),
) -> None:
    ...
    console.print(f"[bold]Web dashboard:[/bold] http://{bind_host}:{bind_port}")
    if settings.web_token:
        console.print("[dim]Protected with web token (see SENTRI_WEB_TOKEN)[/dim]")
    else:
        console.print("[yellow]No web token set — local access only recommended[/yellow]")

    uvicorn.run(
        "sentri.web.app:create_app",
        factory=True,
        host=bind_host,
        port=bind_port,
...
def watch() -> None:
    ...
    if not TelegramClientManager.session_exists(settings.resolved_session_path):
        print_error("Not authenticated. Run 'sentri auth' first.")
        raise typer.Exit(1)

    repo = get_repo()
    enabled = repo.get_all_groups(enabled_only=True)
    if not enabled:
        print_error("No enabled groups. Add groups with 'sentri groups add'.")
        raise typer.Exit(1)
    ...
    if not has_destination:
        print_error(
            "No destination configured. Run 'sentri config set-destination' "
...
def show_config() -> None:
    ...
    groups_list = repo.get_all_groups()
    if groups_list:
        from sentri.cli.helpers import build_groups_table
